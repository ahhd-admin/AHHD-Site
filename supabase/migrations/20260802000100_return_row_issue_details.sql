/*
  # Return per-row detail for skipped locations, not just a count

  merge_google_sheets_data already returns geocode_failed as a count, and
  geocode failures get full detail from geocode_helper.py's
  geocode_failures.json (provider name, DBA, full address, phone,
  website, ACHC source id/url) for the new row-issue notification email.

  locations_skipped (the pre-existing duplicate-data-conflict exception
  handler from 20260731000100) only ever returned a count, with no way
  to know WHICH rows were skipped without a manual DB query. Fixes that
  by collecting provider name + address + achc_source_id for each
  skipped row into a new skipped_details array in the return value, so
  the notification email can name the actual providers, not just a
  number.
*/
