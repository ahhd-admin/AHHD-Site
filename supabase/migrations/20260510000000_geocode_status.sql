/*
  ...
  Adds geocode_status to locations so the admin dashboard can surface
  providers that couldn't be geocoded (e.g. billing not enabled on the
  Google Maps API key) and auto-creates review queue items for them.

  geocode_status values:
    'ok'      — latitude/longitude are present and trusted
    'failed'  — geocoding was attempted but the API returned an error
    'pending' — never attempted (new row, will be tried on next run)
*/

-- ============================================================
-- 1. Add geocode_status column (idempotent)
-- ============================================================

ALTER TABLE locations
  ADD COLUMN IF NOT EXISTS geocode_status text DEFAULT 'pending';

-- Backfill existing rows based on whether coordinates are present
UPDATE locations
...
-- Index for admin dashboard queries ("show me all failed/pending")
CREATE INDEX IF NOT EXISTS locations_geocode_status_idx
  ON locations (geocode_status)
  WHERE geocode_status IN ('failed', 'pending');

-- ============================================================
-- 2. Convenience view for admin: locations needing geocoding
-- ============================================================

CREATE OR REPLACE VIEW locations_needs_geocoding
...
WHERE l.geocode_status IN ('failed', 'pending')
  AND l.source_system = 'achc'
ORDER BY l.state, l.city, l.location_name;

-- ============================================================
-- 3. Update merge_google_sheets_data to handle geocode_status
--    and auto-create review queue items for failed geocodes
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_geocode_status = 'ok' THEN
      -- Resolve any open geocoding review items now that coordinates are available
