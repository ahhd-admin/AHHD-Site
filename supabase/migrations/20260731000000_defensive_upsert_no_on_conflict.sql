/*
  # Rewrite merge_google_sheets_data without relying on ON CONFLICT arbiters

  The live database's unique indexes on organizations.organization_slug,
  locations.achc_source_id, and locations(organization_id, address_line_1,
  city, state, postal_code) apparently don't match what this function's
  ON CONFLICT clauses expect -- confirmed via a real duplicate-key error on
  achc_source_id even though a matching row already existed (the ON CONFLICT
  DO UPDATE should have caught it silently).

  Rather than debug the exact index mismatch, this rewrite replaces every
  ON CONFLICT-based upsert with explicit SELECT-then-branch logic (look the
  row up, UPDATE if found, INSERT if not), which doesn't depend on any
  specific index existing as an arbiter target.
*/
...
BEGIN
  ...
  LOOP
    ...
    -- --------------------------------------------------------
    -- B. Upsert location (explicit lookup, no ON CONFLICT)
    -- --------------------------------------------------------
    ...
    -- Try matching by achc_source_id first (most reliable identity), then
    -- fall back to org+address match.
